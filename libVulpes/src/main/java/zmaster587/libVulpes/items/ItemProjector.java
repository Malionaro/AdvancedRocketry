...
public class ItemProjector extends Item implements IModularInventory, IButtonInventory, INetworkItem, INamedContainerProvider {
	...
	public void mouseEvent(MouseScrollEvent event) {
		if(Minecraft.getInstance().player.isSneaking() && event.getScrollDelta() != 0) {
			...
			if(!stack.isEmpty() && stack.getItem() == this && isValidMachineId(getMachineId(stack))) {
	...
	private void clearStructure(World world, TileMultiBlock tile, @Nonnull ItemStack stack) {
		...
		int directionIndex = getDirection(stack);
		Vector3F<Integer> basepos = getBasePosition(stack);
		Object[][][] structure;
		if(isValidMachineId(prevMachineId) && isValidDirection(directionIndex) && basepos != null) {
	...
	public ActionResultType onItemUseFirst(ItemStack stack, ItemUseContext context) {
		...
		if(player.abilities.isCreativeMode && !player.isSneaking() && isValidMachineId(id) && !world.isRemote && blockList.get(id) == context.getWorld().getBlockState(context.getPos()).getBlock()) {

			Vector3F<Integer> pos = getBasePosition(stack);
			int directionIndex = getDirection(stack);
			if(pos != null && isValidDirection(directionIndex)) {
				Direction direction = Direction.byIndex(directionIndex);
				setYLevel(stack, -1);
		...
		if(!player.isSneaking() && isValidMachineId(id) && world.isRemote) {
	...
	private int getYLevel(ItemStack stack) {
		if(stack.hasTag() && stack.getTag().contains("yOffset")) {
	...
	private int getPrevMachineId(ItemStack stack) {
		if(stack.hasTag() && stack.getTag().contains(IDNAME + "Prev")) {
	...
	private Vector3F<Integer> getBasePosition(ItemStack stack) {
		if(stack.hasTag() && stack.getTag().contains("x") && stack.getTag().contains("y") && stack.getTag().contains("z")) {
	...
	public int getDirection(ItemStack stack) {
		if(stack.hasTag() && stack.getTag().contains("dir")) {
	...
	private boolean isValidMachineId(int id) {
		return id >= 0 && id < machineList.size() && id < blockList.size();
	}
	...
	public void writeDataToNetwork(ByteBuf out, byte id, @Nonnull ItemStack stack) {
		if(id == 0) {
			...
			Vector3F<Integer> pos = getBasePosition(stack);
			if (pos != null && isValidDirection(getDirection(stack))) {
				out.writeInt(pos.x);
				out.writeInt(pos.y);
				out.writeInt(pos.z);
	...
	public void useNetworkData(PlayerEntity player, Dist side, byte id, CompoundNBT nbt, ItemStack stack) {
		if(id == 0) {
			...
			if(isValidMachineId(machineId) && vec != null && isValidDirection(direction)) {
				setYLevel(stack, nbt.getInt("yLevel"));
				rebuildStructure(player.world, this.machineList.get(machineId), stack, vec.x, vec.y, vec.z, Direction.byIndex(direction), true);
			}
		}
		else if(id == 2) {
			int x = nbt.getInt("x");
			...
			if(isValidMachineId(machineId) && isValidDirection(dir))
				rebuildStructure(player.world, this.machineList.get(machineId), stack, x, y, z, Direction.byIndex(dir), true);
		}
	}
