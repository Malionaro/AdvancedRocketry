...
public class ZUtils {
	...
	public static List<ItemStack> readListFromString(String input) {
		...
		for (String entry : entries) {
			...
			if (tagname != null && ItemTags.getCollection().getRegisteredTags().contains(tagname)) {
				List<Item> items = ItemTags.getCollection().getTagByID(tagname).getAllElements();
				if(items.isEmpty()) {
					LibVulpesConfig.logger.warn(parts[0] + " is an empty item tag");
					continue;
				}
	...
	public static LinkedList<Block> readBlockListFromStringList(List<? extends String> input) {
		...
		for(String str : input) {
			...
			if(id != null && BlockTags.getCollection().getRegisteredTags().contains(id)) {
				for(Block block : BlockTags.getCollection().getTagByID(id).getAllElements()) {
				...
				if(block == null)
					LibVulpesConfig.logger.warn("'" + str + "' is not a valid block or block tag");
				else if(!output.contains(block))
					output.add(block);
			}
		}
		return output;
